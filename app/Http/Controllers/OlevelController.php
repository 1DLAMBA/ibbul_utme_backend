...
class OlevelController extends Controller
{
    ...
    public function update_ol1(Update_olevel_Request $request, $reg_number)
    {
        ...
        if ($ol1_exists) {
            ...
            // Update UTME result with card PIN and serial number for O-Level 1
            $utme_result = utme_result::where('reg_number', $reg_number)->first();
            $de_result = DeResult::where('reg_number', $reg_number)->first();

            if ($utme_result) {
                $utme_result->update([
                    'ol1_card_pin' => $request->ol1_card_pin,
                    'ol1_serial_number' => $request->ol1_serial_number,
                ]);
            } elseif ($de_result) {
                // For DE results, we might need to add these fields to the de_results table as well
                // For now, we'll skip this as DE results might not need these fields
            }

            return response()->json('success', 200);
        } else {
            // Create new O-Level record
            $validated_data = $request->validated();
    ...
    public function update_ol2(Update_olevel_Request $request, $reg_number)
    {
        ...
        if ($ol1_exists) {
            ...
            // Update UTME result with card PIN and serial number for O-Level 2
            $utme_result = utme_result::where('reg_number', $reg_number)->first();
            $de_result = DeResult::where('reg_number', $reg_number)->first();

            if ($utme_result) {
                $utme_result->update([
                    'ol2_card_pin' => $request->ol2_card_pin,
                    'ol2_serial_number' => $request->ol2_serial_number,
                ]);
            } elseif ($de_result) {
                // For DE results, we might need to add these fields to the de_results table as well
                // For now, we'll skip this as DE results might not need these fields
